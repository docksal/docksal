...
**macOS only.** Sets the location of the folder on the host machine to mount to VirtualBox. See [file sharing](/core/file-sharing/) for more information.

### IMAGE_SSH_AGENT (global)

`Default: docksal/ssh-agent:1.3`

The Docker image to use for SSH Agent. This contains the keys that are shared among the projects.

### IMAGE_VHOST_PROXY (global)

`Default: docksal/vhost-proxy:1.5` 
...
### DOCKER_VERSION_ALERT_SUPPRESS (global)

When set, suppresses alert about the outdated Docksal version.

### DOCKSAL_ENVIRONMENT (project)
...
### MYSQL_INITDB_SKIP_TZINFO (project)
...
### PGSQL_PORT_MAPPING (project)

`Default:  5432`

The port to use when setting up PostgreSQL.

### APACHE_BASIC_AUTH_USER (project)

Username to use for basic authentication.

### APACHE_BASIC_AUTH_PASS (project)
