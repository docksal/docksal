...
|---------|-----------------|---------|-----------|-----------|
| Linux   | Native          | bind    | **Yes**   | The default option on Linux is "bind" with native FS speed. |
...
| macOS   | Docker for Mac  | bind    | **Yes**   | With bind option your files from host will be shared with Docker for Mac via `osxfs:cached` network filesystem. <br> **Pros:** supports filesystem events and is default for Docker for Mac. <br> **Cons:** pretty slow, 40-50% slower than native FS. |
| macOS   | Docker for Mac  | nfs     | *No*      | You can enable NFS volumes with Docker for Mac manually. <br> **Pros:** much faster than default bind option. NFS only 10-15% slower than native FS. <br> **Cons:** does not support filesystem events (fsnotify). |
| Windows | ANY             | bind    | **Yes**   | On Windows all files are mapped from host to Docker via SMB protocol. <br> **Pros:** relatively fast, 20% overhead as compared to native FS. <br> **Cons:** does not support filesystem events (fsnotify). |
| ANY     | ANY             | unison  | *No*      | With `unison` volumes host files will not be mapped over the network to the VM or `cli`. Instead the files will be synced from your host to the `cli` via an additional Unison container, automatically created per-project. This options might be useful for huge projects where FS performance is a bottleneck. <br> **Pros:** maximum `cli` container FS performance. <br> **Cons:** initial wait for files to sync into `cli`; additional Docksal disk space use; sync delay when you switch git branches; higher CPU usage during files sync; sometimes Unison might 'break'. |
| ANY     | ANY             | none    | *No*      | `none` option is like `unison` option, but without the auto-sync. Host files are not be mapped over the network to the `cli` or VM. Useful for huge projects where FS performance is a bottleneck, but unison is not an option for you. <br> **Pros:** maximum `cli` container FS performance and no wait for the initial sync. <br> **Cons:** you have to sync files manually or checkout and edit files inside `cli` container only. |
