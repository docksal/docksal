...
| macOS   | Docker for Mac              | bind    | **Yes**   | With bind option you files from host will be shared with Docker for Mac via `osxfs:cached` network filesystem. <br> **Pros:** supports filesystem events and is default for Docker for Mac. <br> **Cons:** pretty slow, 40-50% slower than native FS. |
